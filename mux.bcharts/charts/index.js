KISSY.add('mux.bcharts/charts/index',function(S,Brick,Node,Uri){
	
	var module = this;
	var swfPath = new Uri(module.fullpath).resolve('as/case.swf').toString() 
	var $ = Node.all
	var isSWF = 0
	...
	return Brick.extend({
		initializer:function(){
			var self = this
			self.resizeTimer = S.later(function(){
				self.resize()
			},500,true)
			...
			// isSWF = 1
			self.set('w',self.get('el').width())
			self.set('h', self.get('el').height())
	    	...
	    	self.get('el').append(self.get('mainDiv'));
	    	if(isSWF){
	    		...
	    		S.use(self.get('url_svg'),function(S,Case){
	    			self.set('_case', new Case({
	    				...
	    				configData : self.get('configData'),
	    				chartData : self.get('chartData')
	    			})) 
	    		})
	    	}
		},
		...
		resize:function(){
			...
			if(self.get('_resize_index') % 2 == 0){
				if(self.get('w') ==self.get('el').width() && self.get('h') ==self.get('el').height()){
					return
				}
				self._setDivStyle()
				if(!isSWF){
		...
		_isSWF:function(){
			var self = this
			var o = self._xml(self.get('configData'))
		...
		url_svg:{
			value : 'mux.bcharts/charts/js/case'        //case.js   svg版本主入口
		},
		url_swf:{
			value : 'mux.bcharts/charts/as/case'        //case.swf  swf版本主入口
		},
		path_swf:{
			value : swfPath
		},
